...
def sign_up(request):
    ...
    if request.method == 'POST':
        ...
        if password_1!=password:
            res = "两次输入密码不一致！"
            return render(request, 'sign_up.html', {'rename': res})
...
def sign_in(request):
    ...
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            res = "用户名或密码错误！"
            return render(request, 'sign_in.html', {'rename': res})
            #messages.add_message(request, messages.ERROR, '用户名或密码错误！')
    return render(request, 'sign_in.html')

#用户登出逻辑
#当用户没有登陆后，访问被@login_required装饰的视图函数，则会直接跳转到/login/页面
